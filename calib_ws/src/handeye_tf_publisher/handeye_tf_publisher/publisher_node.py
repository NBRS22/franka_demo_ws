...
class HandeyeTfPublisher(Node):
    ...
    def __init__(self):
        ...
        self.orig_frame, self.tracking_base_frame, self.t1_matrix = (self._load_calibration(calib_path))
        ...
        self.get_logger().info(
            ...
            f'({self.orig_frame} -> {self.tracking_base_frame}). '
    ...
    def _load_calibration(self, calib_path):
        ...
        try:
            ...
            # eye_in_hand's calibration output is robot_effector_frame ->
            # tracking_base_frame (the effector moves, the camera is rigid
            # to it); eye_on_base/eye_to_hand's is robot_base_frame ->
            # tracking_base_frame (the camera is fixed in the world) -- same
            # convention easy_handeye2's own handeye_publisher.py uses to
            # pick 'orig'. Needed here too now that this node also serves
            # eye_in_hand calibrations (cf. calib_eye_in_hand): unlike
            # easy_handeye2's publisher, which broadcasts orig ->
            # tracking_base_frame directly, this node always composes
            # through camera_link_frame instead (see _try_publish) --
            # publishing straight to tracking_base_frame (typically
            # camera_color_optical_frame) would give that frame a SECOND,
            # conflicting parent on top of the one the camera driver's own
            # static TF chain already gives it (camera_link_frame is that
            # chain's root, with no parent of its own -- the only frame in
            # it safe to attach an external parent to without a conflict).
            if parameters.get('calibration_type') == 'eye_in_hand':
                orig_frame = parameters['robot_effector_frame']
            else:
                orig_frame = parameters['robot_base_frame']
        ...
        return orig_frame, tracking_base_frame, matrix
    ...
    def _try_publish(self):
        ...
        msg.header.frame_id = self.orig_frame
        ...
        self.get_logger().info(f'Static transform published: {self.orig_frame} -> {self.camera_link_frame}')
