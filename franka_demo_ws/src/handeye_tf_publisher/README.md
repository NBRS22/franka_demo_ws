...
```bash
cd $FP3_ROOT/franka_demo_ws/src
...
# Generate the PDF
source ~/.venvs/apriltag_pdf/bin/activate
python3 $FP3_ROOT/franka_demo_ws/src/handeye_tf_publisher/tools/apriltag_pdf_generator.py \
  --input <tag_image>.png \
  --size 4 \
  --border 2 \
  --label "<family> ID=<id>" \
...
Folder: `$FP3_ROOT/franka_demo_ws/src/handeye_tf_publisher/tags/`
...
ros2 run apriltag_ros apriltag_node \
  ...
  --params-file $FP3_ROOT/franka_demo_ws/src/handeye_tf_publisher/tags/36h11_0_0.04.yaml
```
...
ros2 launch easy_handeye2 calibrate.launch.py \
  ...
  2>&1 | tee /tmp/calib.log | python3 $FP3_ROOT/franka_demo_ws/src/handeye_tf_publisher/tools/watch_calibration_convergence.py
```
