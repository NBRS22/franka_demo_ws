...
class SessionManager:
  ...
  def __init__(
    ...
    self.heartbeat_safety_timeout_seconds = heartbeat_safety_timeout_seconds
  ...
  async def start_session(
    ...
    if self.use_event_driven_heartbeat:
      ...
      async def _on_heartbeat_trigger(event: event_bus.Event):
        ...
        if now - self._last_heartbeat_sent_time < self.heartbeat_min_delay_seconds:
          return
      ...
      async def _on_tool_call(event: event_bus.Event):
        self._last_activity_time = asyncio.get_running_loop().time()
          ...
          while True:
            try:
              await asyncio.wait_for(
                  heartbeat_signal.wait(),
                  timeout=self.heartbeat_safety_timeout_seconds,
              )
            ...
            try:
              if (
                ...
                logger.info("Suppressing %s heartbeat (tool executing)", trigger)
                continue

              if trigger == "safety_timeout":
                now = asyncio.get_running_loop().time()
                time_since_activity = now - self._last_activity_time
                if time_since_activity < self.heartbeat_safety_timeout_seconds:
                  heartbeat_signal.clear()
                  logger.info(
                      "Suppressing safety_timeout — recent activity %.1fs ago",
                      time_since_activity,
                  )
                  continue
                logger.warning(
                    "No model response within %.0fs, sending recovery heartbeat",
                    self.heartbeat_safety_timeout_seconds,
                )
