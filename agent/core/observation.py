  ...
  async def _send_text(self):
    try:
      while True:
        ...
        # Check for user_image sentinel before normal text handling.
        try:
          sentinel = json.loads(text)
          if isinstance(sentinel, dict) and sentinel.get("__type") == "user_image":
            parts = [{
                "inlineData": {
                    "mimeType": sentinel["mime_type"],
                    "data": sentinel["data"],
                }
            }]
            if sentinel.get("caption"):
              parts.append({"text": sentinel["caption"]})
            msg = {
                "clientContent": {
            ...
            await self.session_manager.send_message(msg)
            logging.info("Sent user image to model (mime=%s, caption=%r)",
                         sentinel["mime_type"], sentinel.get("caption"))
            continue
